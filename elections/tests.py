...
class testElection:
    ...
    def __init__(self):
        ...
        e0 = Election.objects.create(
            position="ElectionTest3", vanity=True, end=datetime(2015, 4, 28))
        e1 = Election.objects.create(
            position="ElectionTest4", numSeats=3, end=datetime(2015, 4, 28))
        ...
        for election in self.elections:
            for i in range(election.numSeats * 2):
                c = Candidate.objects.create(
                    election=election,
                    name=str(election.id) + "testingCand" + str(i),
                    blurb="c" + str(i) + "blurb for" + str(election.position) +
                    ".")
        ...
        for election in self.elections:
            ...
            for i in range(15):
                ...
                b = Ballot.objects.create(
                    election=election, voter=self.usr, votes=votestr)
