...
def index(request):
    ...
    return render_to_response(
        'finance/index.html', context_instance=RequestContext(request))
...
def view_all_budgets(request):
    ...
    template_args = {
        ...
        admin,
        'budgets':
        Budget.objects.select_related().order_by('-created_on').filter(
            approved=0)
    }

    return render_to_response(
        'finance/budget-list.html',
        template_args,
...
def json_approved_budgets(request):
    ...
    budgets = Budget.objects.select_related().order_by('-created_on').filter(
        approved=1)
    ...
    for b in object_list:
        resultant.append({
            ...
            b.id,
            'signator':
            unicode(b.organization.signator),
            'organization':
            unicode(b.organization.name),
            'requested':
            unicode("$%.2f" % b.requested),
            'allocated':
            unicode("$%.2f" % b.allocated),
            'created':
            unicode(b.created_on.strftime("%d-%m-%Y")),
            'modified':
            unicode(b.modified_on.strftime("%d-%m-%Y"))
        })
...
def json_unapproved_budgets(request):
    ...
    return HttpResponse(
        serializers.serialize('json', page.object_list),
...
def view_approved_budgets(request):
    ...
    return render_to_response(
        'finance/approved-budget-list.html',
        template_args,
        context_instance=RequestContext(request))
...
def create_budget(request):
    ...
    if not request.user.attended_signator_training:
        template_args = {
            ...
            'Error!',
            'message':
            'Error! You can not create a budget since you did not attend signators training. If you would still like to try and submit a budget please contact the current student body treasurers to make special arrangements.',
            'redirect':
            reverse('finance.views.index')
        }
        return render_to_response(
    ...
    if len(organizations) == 0:
        template_args = {
            ...
            'Error!',
            'message':
            'Error! You can not create a budget unless you register an organization in the organization manager... When you click ok you will be redirected there',
            'redirect':
    ...
    return render(request, 'finance/create-budget.html', template_args)
...
def budget_respond_get(request, budget_id):
    ...
    return render_to_response(
        'finance/budget-response-detail.html',
        template_args,
        context_instance=RequestContext(request))
...
def edit_my_budget_post(request, budget_id):
    ...
    org = Organization.objects.get(
        signator=request.user, id=query['organization'])
...
def budget_search(request):
    ...
    try:
        ...
        return render_to_response(
            'finance/organization-budgets.html',
            context_instance=RequestContext(request))
    ...
    try:
        ...
        orgs = Organization.objects.select_related().order_by('name').filter(
            name__icontains=orgName)
        template_args = {"searchTerm": orgName, "orgs": orgs}
        return render_to_response(
            'finance/organization-disambig.html',
            template_args,
            context_instance=RequestContext(request))
    except Organization.DoesNotExist:
        orgs = []

        template_args = {'searchTerm': orgName}
        return render_to_response(
            'finance/organization-budgets.html',
            template_args,
            context_instance=RequestContext(request))
    ...
    template_args = {'admin': admin, 'budgets': budgets, 'organization': org}
...
def add_signator(request):
    ...
    try:
        ...
        return render_to_response(
            'finance/signator.html', {"currentSignators": currentSignators},
            context_instance=RequestContext(request))
    ...
    template_args = {"result": result, "currentSignators": currentSignators}

    return render_to_response(
        'finance/signator.html',
        template_args,
        context_instance=RequestContext(request))
