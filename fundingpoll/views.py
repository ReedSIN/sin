...
VALID_FACTORS = ['admin', 'fundingpoll', 'student']

ADMIN_FACTORS = ['admin', 'fundingpoll']

TREASURER_FACTORS = ['fundingpoll', 'student']
...
def index(request):
    ...
    if fp_exists and fp.during_registration():
        ...
        fp_reg_orgs = [
            o
            for o in fp.fundingpollorganization_set.filter(
                funding_poll=fp, organization__signator=request.user)
        ]
        reg_orgs = [get_org(o) for o in fp_reg_orgs]
        unreg_orgs = filter(lambda o: o not in reg_orgs,
                            request.user.signator_set.all())
...
def registered(request):
    ...
    fp = get_fp()
    forgs = FundingPollOrganization.objects.select_related().filter(
        funding_poll=fp).order_by('organization__name')

    template_args = {'forgs': forgs}
...
def vote_main(request):
    ...
    vote_set = FundingPollVote.objects.filter(
        voter=request.user, funding_poll=fp)
    if vote_set.exists():
    ...
    forgs = FundingPollOrganization.objects.filter(funding_poll=fp).order_by(
        '?')  #Randomize order
...
def submit_vote(request):
    ...
    voter = request.user
    try:
        FundingPollVote.objects.save_fp_data_list(FundingPollOrganization, fp,
                                                  voter, query)
...
def get_top_40():
    fp = get_fp()
    return fp.fundingpollorganization_set.select_related().order_by(
        '-total_votes')[:40]
...
def my_registrations(request):
    ...
    current_fp = fp.during_registration() or fp.during_voting(
    ) or fp.during_budgets()
    ...
    if request.user.attended_signator_training == False:
        template_args = {
            ...
            'Error!',
            'message':
            'You cannot register an organization because you have not attended Signator Training.',
            'redirect':
            reverse('fundingpoll.views.index')
        }
    ...
    if fp.during_registration():
        ...
        fundingpoll_registered_orgs = [
            o
            for o in fp.fundingpollorganization_set.filter(
                funding_poll=fp, organization__signator=request.user)
        ]
        registered_orgs = [get_org(o) for o in fundingpoll_registered_orgs]
        unregistered_orgs = filter(lambda o: o not in registered_orgs,
                                   request.user.signator_set.all())
        ...
        return render(request, 'fundingpoll/my_registrations.html',
                      template_args)
    else:
        registered_orgs = filter(
            lambda x: x.organization.signator == request.user, get_top_40())
        ...
        return render(request, 'fundingpoll/my_registrations_budget.html',
                      template_args)
...
def save_registration(request):
    ...
    def update_org(d):
        ...
        try:
            f_org = fp.fundingpollorganization_set.select_related().get(
                organization__id=org_id)
        except Exception:
            f_org = FundingPollOrganization(
                organization=org,
                funding_poll=fp,
                total_votes=0,
                top_six=0,
...
def view_all_budgets(request):
    ...
    template_args = {'admin': True, 'budgets': budgets, 'counter': counter()}
...
def view_one_budget(request, budget_id):
    ...
    template_args = {'admin': True, 'budget': budget, 'items': items}
...
def view_results(request):
    ...
    if fp.fundingpollorganization_set.all().count() > 0:
        total_votes = fp.fundingpollorganization_set.all()[:1][
            0].fundingpollvote_set.count()
...
def admin_view_results(request):
    ...
    template_args = {
        "forgs":
        fp.fundingpollorganization_set.select_related().order_by(
            '-total_votes'),
        "counter":
        counter()
    }
...
def preview_budget(request, budget_id):
    ...
    if not admin:
        budget = FundingPollBudget.objects.get(
            organization__organization__signator=request.user, id=budget_id)
    else:
        budget = FundingPollBudget.objects.get(id=budget_id)
    items = budget.fundingpollbudgetitem_set.all()

    template_args = {'admin': admin, 'budget': budget, 'items': items}
...
def edit_budget(request, org_id):
    ...
    org = FundingPollOrganization.objects.select_related().get(
        funding_poll=fp, organization__id=org_id)
...
def save_budget(request, budget_id):
    ...
    template_args = {
        ...
        'Success!',
        'message':
        'Your budget has successfully been saved. You can edit it until budget editing ends at %s'
        % str(fp.end_budgets),
        'redirect':
        url
    }
...
def __process_budget_post(user,
                          budget,
                          total_requested,
                          dictionary,
                          create=True):
...
def csv_budget_list(request):
    ...
    def get_budget(fp_org):
        try:
            budget = fp_org.fundingpollbudget_set.select_related().latest(
                'created_on')
            return {
    ...
    response[
        'Content-Disposition'] = 'attachment; filename=fundingpoll_budget_list.csv'

    writer = UnicodeWriter(response)

    writer.writerow([
        'Organization', 'Signator', 'Administration Desc',
        'Administration Costs', 'Refreshments Desc', 'Refreshments Cost',
        'Entertainment Desc', 'Entertainment Cost', 'Capital Improvments Desc',
        'Capital Improvments Cost', 'Frozen Desc', 'Frozen Cost',
        'Miscellaneous Desc', 'Miscellaneous Cost'
    ])
    for b in budgets:
        if b.get('budget', False):
            if b.get('budget').fundingpollbudgetitem_set.count() == 6:
                admin_item = b['budget'].fundingpollbudgetitem_set.get(
                    name='Administration Costs')
                refresh_item = b['budget'].fundingpollbudgetitem_set.get(
                    name='Refreshments')
                enter_item = b['budget'].fundingpollbudgetitem_set.get(
                    name='Entertainment')
                capi_item = b['budget'].fundingpollbudgetitem_set.get(
                    name='Capital Improvements')
                froz_item = b['budget'].fundingpollbudgetitem_set.get(
                    name='Frozen')
                misc_item = b['budget'].fundingpollbudgetitem_set.get(
                    name='Miscellaneous')
                writer.writerow([
                    b['org'].name, unicode(b['org'].signator), unicode(
                        admin_item.description), unicode(admin_item.requested),
                ...
                writer.writerow([b['org'].name, unicode(b['org'].signator)])
        else:
            writer.writerow([b['org'].name, unicode(b['org'].signator)])
    return response
