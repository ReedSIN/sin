...
def check_user(request):
    ...
    return JsonResponse({'valid': str(exists).lower(), 'name': name})
...
def search_orgs(request):
    ...
    org_list = [{
        ...
        org.id,
        'url':
        reverse('organizations.views.organization_detail', args=(org.id, ))
    } for org in orgs]

    return JsonResponse({'orgs': org_list})
...
def get_signators(request):
    ...
    signator_list = [{
        ...
        signator.id,
        'remove_url':
        reverse('organizations.views.remove_signator', args=(signator.id, ))
    } for signator in signators]

    return JsonResponse({'signators': signator_list})
