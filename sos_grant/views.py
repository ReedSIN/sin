...
def admin_index(request):
    ...
    try:
        grant_dates = SOSGrantDates.objects.all()[
            0]  #.get(end_date__gt=timezone.now())
    except ObjectDoesNotExist:
        grant_dates = None
        messages.info(
            request,
            "There isn't an open grant period right now. You can set the start and end dates for this round of SOS grant applications"
        )
    except MultipleObjectsReturned:
        grant_dates = SOSGrantDates.objects.get(end_date__gt=timezone.now())[0]
        messages.warning(
            request,
            "We found multiple grant periods that are open right now. Please contact a webmaster about this."
        )
...
def admin_create_grant_date(request):
  ...
  '''
    ...
    if request.method == 'GET':
        ...
        return render(request, 'sos_grant/admin_edit_grant_date.html',
                      template_args)
        ...
        if grant_form.is_valid():
            ...
            messages.success(
                request,
                'We created a grant period with the dates you suggested. Thanks!'
            )
            return redirect('sos_grant:index')
        else:
            messages.error(
                request,
                'We weren\'t able to use those dates, please fix them and resubmit.'
            )
            template_args = {"grant_form": grant_form}
            return render(request, 'sos_grant/admin_edit_grant_date.html',
                          template_args)
  ...
  '''
    ...
    if request.method == 'GET':
        try:
            ...
            messages.error(
                request,
                "We couldn\'t find an open grant period, please create one here."
            )
        ...
        try:
            ...
            if grant_form.is_valid():
                ...
                messages.success(
                    request,
                    'The SOS Grant Dates were updated successfully! Party on.')
                return redirect('sos_grant:index')
            else:
                messages.error(
                    request,
                    'We weren\'t able to update those dates, please fix them and resubmit.'
                )
        except:
            messages.error(
                request,
...
def admin_grant_app_list(request):
  ...
  '''
    ...
    try:
        grant_date = SOSGrantDates.objects.get(
            end_date__gt=datetime.datetime.now())
    #otherwise, return the object that most recently ended
    except ObjectDoesNotExist:
        grant_date = SOSGrantDates.objects.filter(
            end_date__lte=datetime.datetime.now()).order_by('end_date')[0]
    #maybe there are no grant dates at all right now
    except:
        grant_date = None
        messages.info(
            "There are no grant dates which are open or have recently ended.")
...
def admin_grant_app_detail(request, grant_app_id):
  ...
  '''
    ...
    return render(request, 'sos_grant/admin_application_detail_view.html',
                  template_args)
...
def manage_application(request):
    ...
    try:
        ...
        messages.error(
            request,
            "You don\'t have an application to manage! Create one here!")
...
def create_application(request):
  ...
  '''
    ...
    try:
        ...
        messages.error(
            request,
            "Sorry, but the SOS grant isn't accepting applications at this time."
        )
        return redirect('sos_grant:index')
    ...
    if request.method == "GET":
        ...
        if sos_form.is_valid():
            ...
            messages.success(
                request,
                'Your application was submitted successfully! Party on.')
            return redirect('sos_grant:index')
        else:
            messages.error(
                request,
                'There were some errors in your application, please fix them and resubmit.'
            )
        template_args = {"sos_form": sos_form, "grant_date": grant_date}
...
def edit_application(request):
    ...
    if request.method == 'GET':
        ...
        return render(request, 'sos_grant/edit_application.html',
                      template_args)
        ...
        try:
            ...
            if sos_form.is_valid():
                ...
                messages.success(
                    request,
                    'Your application was updated successfully! Party on.')
                return redirect('sos_grant:index')
            else:
                messages.error(
                    request,
            ...
            messages.error(
                request,
                'We couldn\'t find an application associated with you... maybe something has gone terribly wrong.'
            )
        #   sos_form = SOSGrantAppForm(request.POST)
        #   if sos_form.is_valid():
        #     grant_app = sos_form.save(commit=False)
...
def delete_application(request):
    ...
    try:
        ...
        messages.error(request,
                       "we couldn't find your application, sorry 'bout that")
    return render(request, 'sos_grant/index.html', template_args)
